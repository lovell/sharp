...
function matchTemplate (referenceImage, callback) {
  this.options.referenceImage = this._createInputDescriptor(referenceImage);
  if (is.fn(callback)) {
    if (this._isStreamInput()) {
      this.on('finish', () => {
        this._flattenBufferIn();
        sharp.matchTemplate(this.options, callback);
      });
    } else {
      sharp.matchTemplate(this.options, callback);
    }
    ...
    if (this._isStreamInput()) {
      return new Promise((resolve, reject) => {
        this.on('finish', function () {
          this._flattenBufferIn();
          sharp.matchTemplate(this.options, (err, stats) => {
            if (err) {
              reject(err);
            } else {
              resolve(stats);
