...
function _createInputDescriptor (input, inputOptions, containerOptions) {
  ...
  if (is.object(inputOptions)) {
    ...
    // Scale (for vector image rendering)
    if (is.defined(inputOptions.scale)) {
      if (is.number(inputOptions.scale)) {
        inputDescriptor.scale = inputOptions.scale;
      } else {
        throw is.invalidParameterError('scale', 'number', inputOptions.scale);
      }
    }
    // limitInputPixels
