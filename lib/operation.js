 ...
 *     // data contains the raw pixel data with only 1 channel
 *     // however, it can be used with composite operation to apply on any image
 *   });
 * 
 * sharp({
 *   create: {
 ...
 *        }
 *      ])
 *      .toBuffer(output, function (err, data, info) {
 *        // now, data is an image with applied noise
 *      })
 *   });
 * 
 * @param {number} [mean=128] - a value between 0 and 10000 representing the mean of pixels in generated image.
 * @param {number} [sigma=30] - a value between 0 and 10000 representing the standard deviation of pixels in generated image.
 * @returns {Sharp}
 * @throws {Error} Invalid parameters
 */
function noise (mean, sigma) {
  if (!is.defined(mean)) {
    this.options.noiseMean = 128;
  } else if (is.number(mean) && is.inRange(mean, 0, 10000)) {
    this.options.noiseMean = mean;
  } else {
    throw is.invalidParameterError('mean', 'number between 0 and 10000', mean);
  }

  if (!is.defined(sigma)) {
    this.options.noiseSigma = 30;
  } else if (is.number(sigma) && is.inRange(sigma, 0, 10000)) {
    this.options.noiseSigma = sigma;
  } else {
    throw is.invalidParameterError('sigma', 'number between 0 and 10000', sigma);
  }
