...
/**
 * Unflatten - apply transparency (0 /255) to the image using a threshold.
 *
 * @example
 * await sharp(rgbInput)
 *   .unflatten({ unflattenThresholds: [255] })
 *   .toBuffer();
 *
 * @example
 * await sharp(rgbInput)
 *   .unflatten({ unflattenThresholds: [128, 240, 130] })
 *   .toBuffer();
 */
function unflatten (options) {
  this.options.unflatten = is.bool(options) ? options : true;
  if (is.object(options)) {
    this.options.unflattenThresholds = options.thresholds;
  }
  return this;
}
