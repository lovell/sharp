 ...
 *   .pipe(outputStream);
 *
 * @param {Array<Array<number>>} matrix - 2x2 affine transformation matrix
 * @param {Object} [options] - if present, is an Object with optional attributes.
 * @param {String|Object} [options.background="#000000"] - parsed by the [color](https://www.npmjs.org/package/color) module to extract values for red, green, blue and alpha.
...
function affine (matrix, options) {
  if (!Array.isArray(matrix) || (matrix.length !== 2 ||
      matrix[0].length !== 2 ||
      matrix[1].length !== 2)) {
    throw is.invalidParameterError('matrix', '2x2 matrix', matrix);
  } else {
    this.options.affineMatrix = [matrix[0][0], matrix[0][1],
  ...
  if (is.defined(options)) {
    if (is.object(options)) {
      ...
      if (is.defined(options.interpolator)) {
        if (is.inArray(options.interpolator, Object.values(enums.interpolators))) {
