 ...
 * @returns {Sharp}
 */
function normalise (lowerBin, upperBin) {
  if(!is.inRange(lowerBin,0,100 ) || !is.inRange(upperBin,0,100)){
    throw new Error('Range must be between 0 and 100 inclusively')
  }
  this.options.normalise = true;
  this.options.normaliseLowerBin = is.number(lowerBin) ? lowerBin : 1;
  this.options.normaliseUpperBin = is.number(upperBin) ? upperBin : 99;
  return this;
}
 ...
 * @returns {Sharp}
 */
function normalize (normalize, lowerBin, upperBin) {
  return this.normalise(normalize, lowerBin, upperBin);
}
