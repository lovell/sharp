...
function toFormat (format, options) {
  ...
  const actualFormat = formats.get(is.object(formatLowerCase) && is.string(formatLowerCase.id) ? formatLowerCase.id : formatLowerCase);
  if (!actualFormat) {
    throw is.invalidParameterError('format', `one of: ${[...formats.keys()].join(', ')}`, formatLowerCase);
  }
