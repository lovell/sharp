 ...
 *   .catch(err => { ... });
 *
 * @param {Object|Function} [optionsOrCallback] The resolveWithObject option if you want to the promise to include info.
 * @param {boolean} [options.resolveWithObject] Resolve the Promise with an Object containing `data` and `info` properties instead of resolving only with `data`.
 * @returns {Promise<Uint8Array>} - when no callback is provided
 */
function toUint8Array (optionsOrCallback) {
  // callback mode
  if (is.fn(optionsOrCallback)) {
    // wrap callback with function that converts to Uint8Array
    return this.toBuffer(optionsOrCallback, (err, data, info) => optionsOrCallback(err, new Uint8Array(data.buffer), info));
  }
  return this.toBuffer(optionsOrCallback).then((res) => {
    if (optionsOrCallback && optionsOrCallback.resolveWithObject) {
      return {
        // creating a new Uint8Array with the ArrayBuffer of a Buffer means the Uint8Array with share underlying memory
