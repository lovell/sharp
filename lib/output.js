 ...
 * @param {string} [options.profile] path to output ICC profile.
 * @returns {Sharp}
 * @throws {Error} Invalid parameters
 */
function withMetadata (options) {
  ...
  if (is.object(options)) {
    ...
    if (is.defined(options.profile)) {
      if (is.string(options.profile)) {
        this.options.withMetadataProfile = path.resolve(__dirname, options.profile);
      } else {
        throw is.invalidParameterError('profile', 'string path to icc profile', options.profile);
      }
    }
  }
  return this;
