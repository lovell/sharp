...
const withMetadata = function withMetadata(withMetadata) {
  ...
  if (is.object(withMetadata)) {
    if (is.defined(withMetadata.orientation)) {
      if (
        is.integer(withMetadata.orientation) &&
        is.inRange(withMetadata.orientation, 1, 8)
      ) {
        this.options.withMetadataOrientation = withMetadata.orientation;
      } else {
        throw new Error(
          'Invalid orientation (1 to 8) ' + withMetadata.orientation
        );
      }
    }
  }
...
const jpeg = function jpeg(options) {
  if (is.object(options)) {
    ...
    options.trellisQuantisation = is.bool(options.trellisQuantization)
    ...
    options.optimiseScans = is.bool(options.optimizeScans)
  ...
  if (is.object(options)) {
    ...
    if (is.defined(options.compressionLevel)) {
      if (
        ...
        throw new Error(
          'Invalid compressionLevel (integer, 0-9) ' + options.compressionLevel
        );
      }
    }
...
const webp = function webp(options) {
  ...
  if (is.object(options) && is.defined(options.alphaQuality)) {
    if (
      ...
      is.inRange(options.alphaQuality, 1, 100)
    ) {
      this.options.webpAlphaQuality = options.alphaQuality;
    } else {
      throw new Error(
        'Invalid webp alpha quality (integer, 1-100) ' + options.alphaQuality
      );
    }
  }
...
const tiff = function tiff(options) {
  ...
  if (is.defined(options) && is.defined(options.compression)) {
    if (
      ...
      is.inArray(options.compression, ['lzw', 'deflate', 'jpeg'])
    ) {
      ...
      throw new Error(
        'Invalid compression option [' +
          options.compression +
          ']. Should be one of: lzw, deflate, jpeg'
      );
    }
  }
  // predictor
  if (is.defined(options) && is.defined(options.predictor)) {
    if (
      ...
      throw new Error(
        'Invalid predictor option [' +
          options.predictor +
          ']. Should be one of: none, horizontal, float'
      );
    }
  }
...
const tile = function tile(tile) {
  if (is.object(tile)) {
      ...
      if (is.integer(tile.overlap) && is.inRange(tile.overlap, 0, 8192)) {
        if (tile.overlap > this.options.tileSize) {
          throw new Error(
            'Tile overlap ' +
              tile.overlap +
              ' cannot be larger than tile size ' +
    ...
    if (is.defined(tile.container)) {
      if (
        is.string(tile.container) && is.inArray(tile.container, ['fs', 'zip'])
      ) {
    ...
    if (is.defined(tile.layout)) {
      if (
        is.string(tile.layout) &&
        is.inArray(tile.layout, ['dz', 'google', 'zoomify'])
      ) {
...
const _updateFormatOut = function _updateFormatOut(formatOut, options) {
  this.options.formatOut = is.object(options) && options.force === false
    ? 'input'
    : formatOut;
  return this;
};
...
/* istanbul ignore next */
const progressive = util.deprecate(
...
/* istanbul ignore next */
const compressionLevel = util.deprecate(
...
/* istanbul ignore next */
const withoutAdaptiveFiltering = util.deprecate(
  function(withoutAdaptiveFiltering) {
    const formatOut = this.options.formatOut;
...
/* istanbul ignore next */
const withoutChromaSubsampling = util.deprecate(
  function(withoutChromaSubsampling) {
    const formatOut = this.options.formatOut;
    this.jpeg({
      chromaSubsampling: withoutChromaSubsampling === false ? '4:2:0' : '4:4:4'
    });
    this.options.formatOut = formatOut;
    return this;
  },
...
/* istanbul ignore next */
const trellisQuantisation = util.deprecate(
  function(trellisQuantisation) {
    const formatOut = this.options.formatOut;
...
/* istanbul ignore next */
const overshootDeringing = util.deprecate(
  function(overshootDeringing) {
    const formatOut = this.options.formatOut;
...
/* istanbul ignore next */
const optimiseScans = util.deprecate(
  function(optimiseScans) {
    const formatOut = this.options.formatOut;
