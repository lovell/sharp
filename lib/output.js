...
const tiff = function tiff (options) {
  ...
  if (is.defined(options) && is.defined(options.compression)) {
    if (is.string(options.compression) && is.inArray(options.compression, ['lzw', 'deflate', 'jpeg'])) {
      this.options.tiffCompression = options.compression;
    } else {
      throw new Error('Invalid compression option [' + options.compression + ']. Should be one of: lzw, deflate, jpeg');
    }
  }
  // predictor
