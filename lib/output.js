...
function keepMetadata () {
  this.options.keepMetadata = 0b1111111;
  return this;
}
 ...
 *
 * CICP metadata describes colour primaries, transfer characteristics,
 * and matrix coefficients per ITU-T H.273. When set, CICP-tagged images
 * will pass through without linearization, preserving the original
 * signal encoding (e.g. PQ, HLG).
 *
 * @example
 * const output = await sharp('hdr-input.avif')
 *   .keepCicp()
 *   .jxl()
 *   .toBuffer();
 *
 ...
 */
function keepCicp () {
  this.options.keepMetadata |= 0b1000000;
  return this;
}
