...
function extend (extend) {
  if (is.integer(extend) && extend > 0) {
    ...
    if (is.defined(extend.extend)) {
      if (is.integer(extendMode[extend.extend])) {
        this.options.extendMode = extendMode[extend.extend];
      } else {
        throw is.invalidParameterError('extend', 'valid extend mode', extend.extend);
      }
    }
  } else {
