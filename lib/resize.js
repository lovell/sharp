 ...
 *   });
 * @param {String} [embed='centre'] - A member of `sharp.gravity` to embed to an edge/corner or `sharp.strategy` to embed dynamically.
 * @returns {Sharp}
 * @throws {Error} Invalid parameters
 */
function embed (embed) {
  ...
  } else if (is.integer(embed) && embed >= strategy.entropy) {
    // Strategy
    this.options.embed = embed;
  } else if (is.string(embed) && is.integer(strategy[embed])) {
    // Strategy (string)
    this.options.embed = strategy[embed];
  } else {
    throw is.invalidParameterError('embed', 'valid embed id/name/strategy', embed);
  }
