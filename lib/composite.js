...
/**
 * Blend modes when compositing images
 * @member
 * @private
 */
const blendMode = {
...
function overlayWith (overlay, options) {
  ...
  if (is.object(options)) {
    ...
    if (is.defined(options.blendMode)) {
      if (is.integer(options.blendMode) && is.inRange(options.blendMode, 0, 11)) {
        this.options.overlayBlendMode = options.blendMode;
      } else if (is.string(options.blendMode) && is.integer(this.constructor.blendMode[options.blendMode])) {
        this.options.overlayBlendMode = this.constructor.blendMode[options.blendMode];
      } else {
        throw new Error('Unsupported overlay blend mode ' + options.blendMode);
      }
    }
  }
  return this;
