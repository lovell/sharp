...
const paths = [
  ...
  `@studiosnack/sharp-${runtimePlatform}/sharp.node`,
  "@studiosnack/sharp-wasm32/sharp.node",
];
...
if (
  sharp &&
  path.startsWith("@studiosnack/sharp-linux-x64") &&
  !sharp._isUsingX64V2()
) {
  const err = new Error(
...
if (sharp) {
  ...
  if (isUnsupportedNodeRuntime()) {
    ...
    help.push(
      ...
      "    npm install @studiosnack/sharp-wasm32"
    );
  }
  if (isLinux && /(symbol not found|CXXABI_)/i.test(messages)) {
    try {
      const {
        config,
      } = require(`@studiosnack/sharp-libvips-${runtimePlatform}/package`);
