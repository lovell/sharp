    ...
    it('should work for webp lossless', function (done) {
      sharp(fixtures.inputPngAlphaPremultiplicationSmall)
        ...
        .toBuffer(function (err, data, info) {
          ...
          assert.strictEqual(true, data.length > 0);
          assert.strictEqual('webp', info.format);
          fixtures.assertSimilar(fixtures.expected('webp-lossless.webp'), data, function (err) {
            if (err) throw err;
    ...
    it('should work for webp near-lossless with correct size', function (done) {
      sharp(fixtures.inputPngAlphaPremultiplicationSmall)
        ...
        .toBuffer(function (err50, data50, info50) {
          ...
          fixtures.assertSimilar(fixtures.expected('webp-near-lossless-50.webp'), data50, function (err) {
            if (err) throw err;
            done();
          });
        });
